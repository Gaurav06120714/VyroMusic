    ...
    >
      ...
      <div className="w-8 shrink-0 flex items-center justify-center">
        {isActive ? (
          ...
          <>
            <span className="text-white/30 text-sm group-hover:hidden">{index ?? ''}</span>
            <svg className="w-4 h-4 text-white hidden group-hover:block fill-current" viewBox="0 0 20 20">
              <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM9.555 7.168A1 1 0 008 8v4a1 1 0 001.555.832l3-2a1 1 0 000-1.664l-3-2z" clipRule="evenodd" />
            </svg>
          </>
        )}
      ...
      {/* Album art */}
      <div className="relative w-10 h-10 shrink-0 rounded-lg overflow-hidden bg-white/5">
      ...
      <div className="flex-1 min-w-0">
        <p className={`text-sm font-medium truncate ${isActive ? 'text-vyro-400' : 'text-white'}`}>
          {track.title}
        </p>
        <p className="text-xs text-white/40 truncate">
          {track.explicit && <span className="bg-white/10 text-white/50 text-[9px] font-bold px-1 py-0.5 rounded mr-1">E</span>}
          {track.artist?.name}
      ...
      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        {user && (
          <button
            onClick={handleLike}
            className={`p-1.5 rounded-lg transition-colors ${liked ? 'text-vyro-400' : 'text-white/30 hover:text-white'}`}
          >
      ...
      {/* Duration */}
      <span className="text-xs text-white/30 font-mono shrink-0 w-10 text-right">
      ...
      {showMenu && (
        <div
          className="absolute right-4 top-10 z-50 glass-card rounded-xl p-1 min-w-[160px] shadow-2xl"
          onClick={e => e.stopPropagation()}
        >
          <button onClick={handleRadio} className="flex items-center gap-2 w-full px-3 py-2 text-sm text-white/70 hover:text-white hover:bg-white/5 rounded-lg">
            <Radio className="w-4 h-4" /> Start Radio
          </button>
          <button onClick={handleLike} className="flex items-center gap-2 w-full px-3 py-2 text-sm text-white/70 hover:text-white hover:bg-white/5 rounded-lg">
            <Heart className="w-4 h-4" /> {liked ? 'Unlike' : 'Like'}
          </button>
        </div>
      )}
    </div>
